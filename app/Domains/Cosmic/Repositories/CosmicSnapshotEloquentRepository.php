...
namespace App\Domains\Cosmic\Repositories;
...
class CosmicSnapshotEloquentRepository implements CosmicSnapshotRepositoryInterface
{
    public function saveSnapshot(int $worldId, WorldSnapshot $snapshot): void
    {
        CosmicSnapshot::updateOrCreate(
            ...
            [
                ...
                // Civilization
                'civ_knowledge' => $snapshot->civilization->collectiveKnowledge,
                'civ_ritual_coherence' => $snapshot->civilization->ritualCoherence,
                'civ_tech_level' => $snapshot->civilization->technologicalLevel,
                'civ_faction_stability' => $snapshot->civilization->factionStability,
    ...
    private function modelToSnapshot(CosmicSnapshot $model): WorldSnapshot
    {
        return new WorldSnapshot(
            ...
            civilization: new CivilizationState(
                collectiveKnowledge: $model->civ_knowledge,
                ritualCoherence: $model->civ_ritual_coherence,
                technologicalLevel: $model->civ_tech_level,
                factionStability: $model->civ_faction_stability,
    ...
    private function hydrateSocialClasses(?array $data): array
    {
        ...
        return array_map(function ($c) {
            return new \App\Domains\Cosmic\ValueObjects\SocialClass(
                type: \App\Domains\Cosmic\Enums\SocialClassType::from($c['type']),
                power: (float) $c['power'],
