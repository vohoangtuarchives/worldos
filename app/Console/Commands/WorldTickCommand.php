...
final class WorldTickCommand extends Command
{
    protected $signature = 'world:tick 
                            {--world-id= : Specific world ID to tick}
                            {--count=1 : Number of ticks to run}
                            {--force : Force tick even if world is not autonomous}
    ...
    private function runTicks(WorldAggregate $world, array $characters, int $count, bool $isDryRun): TickSummary
    {
        ...
        for ($i = 1; $i <= $count; $i++) {
            ...
            if ($isDryRun) {
                ...
                $this->line("  Would check character survival for " . count($characters) . " characters");
                $summary->addDryRun();
                continue;
            }
            ...
            $characters = array_map(function ($character) use ($result) {
...
final class TickSummary
{
    ...
    public function addError(string $worldId, string $error): void
    {
    ...
    public function getFirstWorldId(): ?string
    {
        return array_key_first($this->worldSummaries);
    }

    public function getWorldSummary(string $worldId): array
    {
