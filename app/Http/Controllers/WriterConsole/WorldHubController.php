...
class WorldHubController extends Controller
{
    ...
    private function loadCosmicData(World $world): array
    {
        ...
        // Metrics
        $trajectory = $world->cosmicSnapshots()->latest('year')->limit(50)->get()->reverse()->values();
    ...
    private function calculateDistance($cosmic, $attractor): float
    {
        ...
        $dEntropy = ($cosmic->entropy ?? 0) - ($eq['entropy'] ?? 0);
        $dStability = ($cosmic->stability ?? 0) - ($eq['stability'] ?? 0);
        $dEnergy = ($cosmic->energy ?? 0) - ($eq['energy'] ?? 0);
