...
- **Công dụng**: Truyện dài kỳ (serial): tạo series, sinh chương tiếp (generateNextChapter), Story Bible, arcs. Đọc state Universe (khi series có universe_id) chỉ để build context; không ghi vào World/Universe. Narrative_driven_state lưu trong narrative_state; Causality Bridge (StoryEventExtractor + WorldMutationPolicy) cập nhật narrative_driven_state từ nội dung chương. **WorldOS 2.0 Clean**: Khi `mutation.narrative_affects_via_pressure` bật, NarrativeToUniverseAdapter gọi **NarrativePressureBridgeInterface::injectPressure(PressureSignal)** thay vì UniverseMutationService — narrative tạo pressure cho phase transition, không mutate vector trực tiếp. Contract: DTO PressureSignal (universe_id, intensity, source); stub NullNarrativePressureBridge; binding trong AppServiceProvider.
