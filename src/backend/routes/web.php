...
use App\Http\Controllers\CivilizationForgeController;
...
// Civilization Forge Routes
Route::prefix('civilization-forge')->name('civilization-forge.')->group(function () {
    Route::get('/', [CivilizationForgeController::class, 'index'])->name('index');
    Route::post('/generate', [CivilizationForgeController::class, 'generate'])->name('generate');
    Route::get('/anchors', [CivilizationForgeController::class, 'anchors'])->name('anchors');
    Route::get('/history', [CivilizationForgeController::class, 'history'])->name('history');
    Route::get('/detail/{id}', [CivilizationForgeController::class, 'detail'])->name('detail');
});
...
Route::middleware(['auth', 'verified'])->prefix('writer')->name('writer.')->group(function () {
    ...
    Route::post('/worlds/{worldId}/autonomous/tick', [\App\Http\Controllers\WriterConsole\WorldHubController::class, 'autonomousTick'])->name('worlds.autonomous.tick');
    Route::post('/worlds/{worldId}/autonomous/toggle', [\App\Http\Controllers\WriterConsole\WorldHubController::class, 'toggleAutonomous'])->name('worlds.autonomous.toggle');

    // Material Library (New)
    Route::get('/materials/library', [\App\Http\Controllers\Writer\MaterialLibraryController::class, 'index'])->name('materials.library');
    Route::post('/materials/generate', [\App\Http\Controllers\Writer\MaterialLibraryController::class, 'generate'])->name('materials.generate');
    Route::post('/materials/save/{id}', [\App\Http\Controllers\Writer\MaterialLibraryController::class, 'save'])->name('materials.save');
    
    // Genesis (Incarnation)
    Route::post('/genesis/{id}', [\App\Http\Controllers\Writer\WorldGenesisController::class, 'store'])->name('genesis.incarnate');
    ...
    Route::prefix('god-console/{worldId}')->name('god_console.')->group(function () {
        Route::get('/', [\App\Http\Controllers\Writer\GodConsoleController::class, 'index'])->name('index');
        Route::get('/metrics', [\App\Http\Controllers\Writer\GodConsoleController::class, 'getMetrics'])->name('metrics');
        Route::post('/intervene', [\App\Http\Controllers\Writer\GodConsoleController::class, 'intervene'])->name('intervene');
    });
