...
<div class="space-y-6">
    {{-- Autonomous Controls --}}
    <div class="bg-gradient-to-r from-purple-900/40 to-indigo-900/40 rounded-lg border border-purple-500/30">
        <div class="px-5 py-4 border-b border-purple-500/20 flex justify-between items-center">
            <h3 class="text-base font-semibold text-white flex items-center">
                <span class="mr-2">🤖</span> Autonomous Simulation
            </h3>
            <span class="inline-flex items-center rounded-full px-2 py-1 text-xs font-medium {{ $world->autonomous ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-gray-700 text-gray-400' }}">
                {{ $world->autonomous ? 'AUTONOMOUS ACTIVE' : 'MANUAL MODE' }}
            </span>
        </div>
        <div class="p-5 flex flex-wrap gap-4 items-center">
            {{-- Toggle Button --}}
            <form method="POST" action="{{ route('writer.worlds.autonomous.toggle', $world->id) }}">
                @csrf
                <button type="submit" class="flex items-center space-x-2 rounded-md px-4 py-2 text-sm font-semibold text-white transition-all shadow-lg
                    {{ $world->autonomous 
                        ? 'bg-red-600 hover:bg-red-500 ring-1 ring-red-400' 
                        : 'bg-green-600 hover:bg-green-500 ring-1 ring-green-400' }}">
                    <span>{{ $world->autonomous ? '🛑 Stop Autonomous' : '🟢 Start Autonomous' }}</span>
                </button>
            </form>

            {{-- Manual Tick --}}
            <form method="POST" action="{{ route('writer.worlds.autonomous.tick', $world->id) }}">
                @csrf
                <button type="submit" class="flex items-center space-x-2 rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-500 transition-all shadow-lg ring-1 ring-indigo-400">
                    <span>⏱️ Force 1 Tick</span>
                </button>
            </form>

            <div class="text-xs text-gray-400 max-w-md ml-auto">
                Controls the autonomous evolution engine. When active, the world will evolve automatically via the background scheduler.
            </div>
        </div>
    </div>
    {{-- Simulation Controls --}}
