<?php

declare(strict_types=1);

namespace WorldOS\Legacy\Presentation\Http\Controllers\Runtime;

use Illuminate\Http\JsonResponse;
use Illuminate\Http\Request;
use WorldOS\Legacy\Application\Runtime\CreateUniverse\CreateUniverseCommand;
use WorldOS\Legacy\Application\Runtime\CreateUniverse\CreateUniverseHandler;

final class CreateUniverseController
{
    public function __construct(
        private readonly CreateUniverseHandler $createUniverseHandler,
    ) {
    }

    public function __invoke(Request $request): JsonResponse
    {
        $name = (string) $request->input('name', '');
        $worldId = (string) $request->input('world_id', '');
        $sagaId = (string) $request->input('saga_id', '');
        
        if ($name === '' || $worldId === '' || $sagaId === '') {
            return response()->json(['message' => 'name, world_id, and saga_id are required'], 422);
        }
        $command = new CreateUniverseCommand($name, $worldId, $sagaId);
        $result = $this->createUniverseHandler->handle($command);
        return response()->json([
            'id' => $result->id,
            'name' => $result->name,
        ], 201);
    }
}
