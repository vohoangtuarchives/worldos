...
use WorldOS\Legacy\Application\Cosmology\Services\CosmicEvolutionService; // Added this use statement as per instruction
...
class WorldEvolutionPipeline
{
    ...
    private function calculateSocialModifiers(CivilizationState $civ): array
    {
        ...
        foreach ($civ->socialClasses as $class) {
            switch ($class->type) {
                case \WorldOS\Legacy\Domain\Cosmology\Enums\SocialClassType::MERCHANT:
                    // High Merchant power boosts efficiency
                    $mods['efficiency_bonus'] += $class->power * 0.2;
                    break;
                case \WorldOS\Legacy\Domain\Cosmology\Enums\SocialClassType::WARRIOR:
                    // Disgruntled warriors hurt stability, happy ones help
                    $mods['stability_modifier'] += ($class->contentment - 0.5) * $class->power * 0.1;
                    break;
                case \WorldOS\Legacy\Domain\Cosmology\Enums\SocialClassType::INTELLECTUAL:
                    // Intellectuals boost knowledge growth
                    $mods['knowledge_growth_factor'] += $class->power * 0.5;
                    break;
                case \WorldOS\Legacy\Domain\Cosmology\Enums\SocialClassType::NOBILITY:
