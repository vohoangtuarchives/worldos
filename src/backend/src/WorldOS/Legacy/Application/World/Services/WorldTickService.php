...
class WorldTickService
{
    ...
    private function generateHistory(World $world, int $tick): void
    {
        ...
        if (rand(0, 100) < 10) {
            $ledger = app(\WorldOS\Legacy\Application\World\Services\WorldEventLedger::class);
        ...
        if (rand(0, 100) < 2) {
            $shockGen = app(\WorldOS\Legacy\Application\World\Services\ShockEventGenerator::class);
    ...
    private function checkCanonization(World $world): void
    {
        ...
        $metaLayer = new \WorldOS\Legacy\Domain\Meta\Aggregates\MetaLayer();
