...
readonly class NarrativeWeight
{
    ...
    {
        return new self(
            (float) ($data['story_importance'] ?? 0.0),
            (float) ($data['plot_criticality'] ?? 0.0),
            (float) ($data['emotional_impact'] ?? 0.0),
            (float) ($data['completion_percentage'] ?? 0.0),
            (bool) ($data['is_main_character'] ?? false),
        );
    }
