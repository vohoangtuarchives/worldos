...
use WorldOS\Domains\Evolution\Enums\PowerStage;
...
final class CivilizationSnapshot
{
    ...
    public function __construct(
        ...
        /** @var \WorldOS\Domains\Evolution\ValueObjects\SocialClass[] */
    ...
    public static function defaultObservation(int $year = 0): self
    {
        return new self(
            ...
            yearsInPhase: 0,
            socialClasses: [
                new \WorldOS\Domains\Evolution\ValueObjects\SocialClass(\WorldOS\Domains\Evolution\Enums\SocialClassType::NOBILITY, 0.8, 0.9, 0.05),
                new \WorldOS\Domains\Evolution\ValueObjects\SocialClass(\WorldOS\Domains\Evolution\Enums\SocialClassType::PRIESTHOOD, 0.6, 0.8, 0.05),
                new \WorldOS\Domains\Evolution\ValueObjects\SocialClass(\WorldOS\Domains\Evolution\Enums\SocialClassType::WARRIOR, 0.5, 0.7, 0.10),
                new \WorldOS\Domains\Evolution\ValueObjects\SocialClass(\WorldOS\Domains\Evolution\Enums\SocialClassType::MERCHANT, 0.2, 0.5, 0.05),
                new \WorldOS\Domains\Evolution\ValueObjects\SocialClass(\WorldOS\Domains\Evolution\Enums\SocialClassType::PEASANTRY, 0.1, 0.4, 0.75),
                new \WorldOS\Domains\Evolution\ValueObjects\SocialClass(\WorldOS\Domains\Evolution\Enums\SocialClassType::INTELLECTUAL, 0.01, 0.9, 0.01),
            ],
    ...
    public function evolve(
        \WorldOS\Domains\Cosmology\ValueObjects\PhysicalLaws $laws,
        ...
        $constraintEngine = new \WorldOS\Domains\Evolution\Services\RealityConstraintEngine();
        $dynamicsAnalyzer = new \WorldOS\Domains\Evolution\Services\DynamicsAnalyzer();
        $attractorService = new \WorldOS\Domains\Evolution\Services\AttractorService();
        $phaseEngine = new \WorldOS\Domains\Evolution\Services\PhaseEngine();
        $kernel = new \WorldOS\Domains\Evolution\Mathematics\DynamicalKernel();
        $narrativeEngine = new \WorldOS\Domains\Evolution\Services\NarrativeEngine();
        $heroSystem = new \WorldOS\Domains\Evolution\Services\HeroSystem();
        
        $phaseForces = $phaseEngine->getPhaseForces($this->historyPhase);
        
        $state = \WorldOS\Domains\Evolution\ValueObjects\StateVector::fromSnapshot($this);
        $prevSnapshot = $this;
        ...
        for ($i = 0; $i < $deltaYears; $i++) {
            ...
            $extForcesFromHero = [];
            if ($heroSystem->checkEmergence($totalTension, 1)) { 
            ...
            foreach(\WorldOS\Domains\Evolution\ValueObjects\StateVector::KEYS as $idx => $keyName) {
            ...
            $mpIdx = array_search('mp', \WorldOS\Domains\Evolution\ValueObjects\StateVector::KEYS);
            ...
            $stab = $nextState->values[array_search('stability', \WorldOS\Domains\Evolution\ValueObjects\StateVector::KEYS)];
            $p = $nextState->values[array_search('prosperity', \WorldOS\Domains\Evolution\ValueObjects\StateVector::KEYS)];
            $ie = $nextState->values[array_search('ie', \WorldOS\Domains\Evolution\ValueObjects\StateVector::KEYS)];
            ...
            // $Resilience = 0.5 + 0.3 * CulturalLegacy - 0.2 * T_l
            // Legacy is stable, but $r fluctuates according to stress/recovery within this bound?
            // Actually, the user asked for: Resilience = 0.5 + 0.3 * CulturalLegacy - 0.2 * T_l
            // Let's set it as a moving target or just calculate it directly.
            $legacyIdx = array_search('legacy', \WorldOS\Domains\Evolution\ValueObjects\StateVector::KEYS);
            $legacyVal = $state->values[$legacyIdx] ?? 0.1;
            
            $resilienceTarget = 0.5 + 0.3 * $legacyVal - 0.2 * $tensionLong;
            ...
            $r = (1 - 0.05) * $r + 0.05 * $resilienceTarget;
            $r = max(0.0, min(1.0, $r + ($resilienceRecovery - $resilienceStress * $graceFactor)));
    ...
    public static function fromArray(array $data): self
    {
        return new self(
            ...
            socialClasses: array_map(
                fn($c) => \WorldOS\Domains\Evolution\ValueObjects\SocialClass::fromArray($c), 
                $data['social_classes'] ?? []
            ),
