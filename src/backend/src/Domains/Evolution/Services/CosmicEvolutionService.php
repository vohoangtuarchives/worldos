...
class CosmicEvolutionService
{
     ...
     */
    public function step(\WorldOS\Domains\Cosmology\ValueObjects\PhysicalLaws $laws, CosmicState $currentState, float $civilizationResonance = 0.0, int $deltaYears = 100, float $entropyFeedback = 0.0): CosmicState
    {
     ...
     */
    private function selectTargetAttractor(CosmicState $state, \WorldOS\Domains\Evolution\AttractorAggregate $currentAttractor): string
    {
        ...
        $catalog = \WorldOS\Domains\Evolution\ValueObjects\Attractor::catalog();
        ...
        foreach ($catalog as $candidateCode => $candidate) {
            ...
            $equilibriumEnergy = $centroid[\WorldOS\Domains\Evolution\ValueObjects\WorldStateVector::DIMENSION_ORDER] ?? 0.5; // using Order as proxy for energy here in V3
