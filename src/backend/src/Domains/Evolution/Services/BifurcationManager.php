...
class BifurcationManager
{
    ...
    public function __construct(
        private \WorldOS\Domains\Evolution\Contracts\AttractorRepositoryInterface $attractorRepository,
        private ?\WorldOS\Domains\Evolution\Services\MorphingEngine $morphEngine = null
    ) {
    }
     ...
     */
    private function minorBifurcation(WorldSnapshot $snapshot, \WorldOS\Domains\Evolution\AttractorAggregate $from, float $force): array
    {
     ...
     */
    private function minorBifurcationWithMorph(WorldSnapshot $snapshot, \WorldOS\Domains\Evolution\AttractorAggregate $from, string $targetCode, float $force): array
    {
        $cosmic = $snapshot->cosmic;
     ...
     */
    private function majorBifurcation(WorldSnapshot $snapshot, \WorldOS\Domains\Evolution\AttractorAggregate $from, float $force): array
    {
        $cosmic = $snapshot->cosmic;
        ...
        $centroid = [
            \WorldOS\Domains\Evolution\ValueObjects\WorldStateVector::DIMENSION_ENTROPY => round($cosmic->entropy, 2),
            \WorldOS\Domains\Evolution\ValueObjects\WorldStateVector::DIMENSION_ORDER => round($cosmic->energy, 2),
        ];

        $newAttractorVO = new \WorldOS\Domains\Evolution\ValueObjects\Attractor(
            $newCode,
            $centroid,
            $centroid
        );

        $newAttractor = \WorldOS\Domains\Evolution\AttractorAggregate::fromAttractor($newAttractorVO, $cosmic->year);
     ...
     */
    private function selectClosestAttractor(CosmicState $state, \WorldOS\Domains\Evolution\AttractorAggregate $current): string
    {
        ...
        foreach ($catalog as $candidateCode => $candidate) {
            ...
            $equilibriumEntropy = $centroid[\WorldOS\Domains\Evolution\ValueObjects\WorldStateVector::DIMENSION_ENTROPY] ?? 0.5;
            $equilibriumEnergy = $centroid[\WorldOS\Domains\Evolution\ValueObjects\WorldStateVector::DIMENSION_ORDER] ?? 0.5;
