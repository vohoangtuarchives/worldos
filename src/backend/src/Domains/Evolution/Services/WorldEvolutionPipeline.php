...
use WorldOS\Domains\Evolution\ValueObjects\EnvironmentState;
...
use WorldOS\Domains\Evolution\Enums\WorldPhase;
...
class WorldEvolutionPipeline
{
    ...
    public function __construct(
        ...
        private \WorldOS\Domains\Material\Services\MaterialEvolutionService $materialService,
        private \WorldOS\Domains\Evolution\Contracts\EntropyLedgerInterface $entropyLedger,
        private PerturbationGenerator $perturbationGenerator, // NEW
        private ?\WorldOS\Domains\Evolution\Contracts\AttractorRepositoryInterface $attractorRepository = null,
     ...
     */
    public function step(\WorldOS\Domains\Cosmology\World $world, WorldSnapshot $current, float $agentPerturbation = 0.0, int $deltaYears = 100, array $externalModifiers = []): WorldSnapshot
    {
        ...
        $tempWorld = new \WorldOS\Domains\Evolution\WorldState($world->getId(), $nextCosmic, $nextEnvironment, $current->year + $deltaYears);
        ...
        $stateMachine = new \WorldOS\Domains\Evolution\Services\HistoryStateMachine();
        ...
        if ($transcendenceEvent) {
            ...
            $phase = \WorldOS\Domains\Evolution\Enums\CivilizationPhase::ILLUMINATION;
        }

        if ($lifecycleState === CivilizationLifecycleState::COLLAPSE || $macroState === \WorldOS\Domains\Evolution\Services\HistoryStateMachine::STATE_COLLAPSE) {
             $this->lastStepEvents[] = [
                'id' => \Illuminate\Support\Str::uuid()->toString(),
        ...
        $nextPhase = $isTranscendence ? \WorldOS\Domains\Evolution\Enums\CivilizationPhase::ILLUMINATION : $this->phaseEngine->determinePhase($nextCivilization);
        
        // If we are already illuminated, keep it until tension gets too high
        if ($currentPhase === \WorldOS\Domains\Evolution\Enums\CivilizationPhase::ILLUMINATION && !$isTranscendence) {
            if ($nextCivilization->narrativeTension > 0.4 || $nextCivilization->internalEntropy > 0.5) {
                ...
                // Stay illuminated
                $nextPhase = \WorldOS\Domains\Evolution\Enums\CivilizationPhase::ILLUMINATION;
            }
        }
     ...
     */
    private function applyTrauma(\WorldOS\Domains\Evolution\ValueObjects\CivilizationResidual $residual, array $trauma): \WorldOS\Domains\Evolution\ValueObjects\CivilizationResidual
    {
