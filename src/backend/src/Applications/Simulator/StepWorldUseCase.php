...
class StepWorldUseCase
{
    public function __construct(
        ...
        private \WorldOS\Domains\Cosmology\WorldRepository $worldRepo, // Thêm WorldRepo
    ...
    public function execute(string $worldId, int $ticks = 1, int $deltaYears = 1): array
    {
        ...
        if (!$world) {
            ...
            $world = new \WorldOS\Domains\Cosmology\World($worldId, "World {$worldId}", \WorldOS\Domains\Cosmology\Archetype::HIGH_FANTASY);
        }
        ...
        for ($i = 0; $i < $ticks; $i++) {
            ...
            $currentSnapshot = new WorldSnapshot(
                ...
                worldField: \WorldOS\Domains\Evolution\ValueObjects\WorldField::default(),
                worldPhase: \WorldOS\Domains\Evolution\Enums\WorldPhase::CIVILIZATIONAL_AGE,
                lifeState: \WorldOS\Domains\Evolution\ValueObjects\LifeState::primordial(),
