...
use Tuzy\Domain\Narrative\Contracts\ChroniclerInterface;
...
class LLMChronicler implements ChroniclerInterface
{
    ...
    private ?\Tuzy\Domain\Genre\GenreRegistry $registry;

    public function __construct(NarrativeBridge $bridge, ?LLMProvider $llm = null, ?\Tuzy\Domain\Genre\GenreRegistry $registry = null)
    {
        $this->bridge = $bridge;
        ...
        $this->registry = $registry ?? (app()->bound(\Tuzy\Domain\Genre\GenreRegistry::class) ? app(\Tuzy\Domain\Genre\GenreRegistry::class) : null);
    }
