...
use Tuzy\Domain\Material\MaterialSeeder;
use Tuzy\Domain\Material\MaterialWorldBridge;
use Tuzy\Domain\Material\MaterialArchetypeCoupler;
...
class SagaRunner
{
    ...
    private \Tuzy\Application\World\Services\WorldEventLedger $ledger;
    private \Tuzy\Application\World\Services\StageTransitionEngine $transitionEngine;
    private \Tuzy\Application\Saga\Services\SagaDirector $director;
    ...
    public function __construct(
        ...
        \Tuzy\Application\Saga\Services\SagaDirector $director = null,
        ...
        $this->ledger = new \Tuzy\Application\World\Services\WorldEventLedger();
        $this->transitionEngine = new \Tuzy\Application\World\Services\StageTransitionEngine(
            $this->ledger,
            app(\Tuzy\Domain\Power\PowerStageRegistry::class)
        );
        $this->director = $director ?? app(\Tuzy\Application\Saga\Services\SagaDirector::class);
    ...
    private function simulateWorld(SagaWorld $sagaWorld): void
    {
        ...
        $authorRegistry = app(\Tuzy\Application\Saga\Author\AuthorRegistry::class);
        ...
        $narrativeAssembler = app(\Tuzy\Domain\Saga\CausalNarrativeAssembler::class);
        $agentOrchestrator = app(\Tuzy\Application\Faction\Services\FactionAgent::class);
        $conflictResolver = app(\Tuzy\Application\Faction\Services\ConflictResolver::class);
        ...
        while ($world->current_time < $maxYears) {
            ...
            if ($this->evolutionPipeline) {
                ...
                if ($world->origin_type === 'vietnamese') {
                    ...
                    $realmService = app(\Tuzy\Application\Vietnamese\Services\RealmContactService::class);
    ...
    public function calculateGlobalInstability(World $world): float
    {
        ...
        $impactService = app(\Tuzy\Application\History\Services\ScarImpactService::class);
    ...
    {
        $weights = $this->archetypePool->getWeightsForWorld($world);
        $archetypes = \Tuzy\Domain\CognitiveKernel\Archetype::all()->keyBy('key');
    ...
    private function createWorld(Saga $saga, ?array $legacy): World
    {
        ...
        if ($originType === 'vietnamese') {
             return app(\Tuzy\Application\Vietnamese\Services\VietnameseOriginService::class)
        ...
        $preset = app(
            \Tuzy\Application\Saga\Services\GenesisPresetService::class
        )->find($presetKey) ?? [];
        ...
        if (!empty($preset)) {
            app(\Tuzy\Application\World\Services\WorldPowerProfileService::class)
