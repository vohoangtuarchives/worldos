...
/**
 * ConstraintEngine â€” enforces hard limits on Meta-AI proposals.
 *
 * From RFC Â§7.5:
 *   Safety Guardrails:
 *   1. Max memory bias: ||MemoryBias|| â‰¤ 0.25
 *   2. Max style weight delta: |Î´| â‰¤ 0.15 per adjustment
 *   3. Archetype redundancy check (cosine similarity â‰¥ 0.92 â†’ reject)
 *   4. Min stability margin before applying style changes
 *   5. Max proposal frequency (cooldown)
 *
 * This engine VETOES unsafe proposals. It cannot approve â€” only block.
 */
class ConstraintEngine
{
    ...
    public function validateEmergentArchetype(array $proposedSemantic, SemanticProjector $projector): array
    {
        if ($projector->isRedundant($proposedSemantic, self::ARCHETYPE_REDUNDANCY_THRESHOLD)) {
            return [
                'valid' => false,
                'violation' => 'Proposed archetype too similar to existing (cosine â‰¥ ' . self::ARCHETYPE_REDUNDANCY_THRESHOLD . ')',
            ];
        }
