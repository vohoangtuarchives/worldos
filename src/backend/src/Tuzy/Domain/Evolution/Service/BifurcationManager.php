...
class BifurcationManager
{
    ...
    public function __construct(
        private \Tuzy\Domain\Evolution\Contract\AttractorRepositoryInterface $attractorRepository,
        private ?\Tuzy\Domain\Evolution\Service\MorphingEngine $morphEngine = null
    ) {
    }
     ...
     */
    private function minorBifurcation(WorldSnapshot $snapshot, \Tuzy\Domain\Evolution\Entity\AttractorAggregate $from, float $force): array
    {
     ...
     */
    private function minorBifurcationWithMorph(WorldSnapshot $snapshot, \Tuzy\Domain\Evolution\Entity\AttractorAggregate $from, string $targetCode, float $force): array
    {
        $cosmic = $snapshot->cosmic;
     ...
     */
    private function majorBifurcation(WorldSnapshot $snapshot, \Tuzy\Domain\Evolution\Entity\AttractorAggregate $from, float $force): array
    {
        $cosmic = $snapshot->cosmic;
        ...
        $centroid = [
            \Tuzy\Domain\Evolution\ValueObject\WorldStateVector::DIMENSION_ENTROPY => round($cosmic->entropy, 2),
            \Tuzy\Domain\Evolution\ValueObject\WorldStateVector::DIMENSION_ORDER => round($cosmic->energy, 2),
        ];

        $newAttractorVO = new \Tuzy\Domain\Evolution\ValueObject\Attractor(
            $newCode,
            $centroid,
            $centroid
        );

        $newAttractor = \Tuzy\Domain\Evolution\Entity\AttractorAggregate::fromAttractor($newAttractorVO, $cosmic->year);
     ...
     */
    private function selectClosestAttractor(CosmicState $state, \Tuzy\Domain\Evolution\Entity\AttractorAggregate $current): string
    {
        ...
        foreach ($catalog as $candidateCode => $candidate) {
            ...
            $equilibriumEnergy = $centroid[\Tuzy\Domain\Evolution\ValueObject\WorldStateVector::DIMENSION_ORDER] ?? 0.5;
