...
class WorldTickService
{
    ...
    private function generateHistory(World $world, int $tick): void
    {
        ...
        if (rand(0, 100) < 10) {
            $ledger = app(\App\Domains\World\Services\WorldEventLedger::class);
        ...
        if (rand(0, 100) < 2) {
            $shockGen = app(\App\Domains\World\Services\ShockEventGenerator::class);
    ...
    private function checkCanonization(World $world): void
    {
        ...
        $metaLayer = new \App\Domains\Meta\Aggregates\MetaLayer();
