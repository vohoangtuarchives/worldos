...
class PhysicsProfile implements Arrayable
{
    /**
    ...
    public function __construct(
        public readonly float $instability_rate,
        public readonly float $decay_rate,
        public readonly float $mutation_chance,
        public readonly float $dimensional_permeability,
        public readonly float $entropy_cap,
        public readonly float $energy_conservation_factor,
    ) {
        $this->validate();
    }
    ...
    public static function standard(): self
    {
        return new self(
            ...
            entropy_cap: 1000.0,
            energy_conservation_factor: 0.8 // 20% loss to entropy
        );
    }
    ...
    {
        return new self(
            instability_rate: 0.1, // Void is stable in high entropy
            decay_rate: 0.01, // Very efficient
            mutation_chance: 0.5, // Extremely mutable
            dimensional_permeability: 10.0, // Open to everything
            entropy_cap: 10000.0, // Huge capacity
    ...
    {
        return new self(
            instability_rate: $data['instability_rate'] ?? 1.0,
            decay_rate: $data['decay_rate'] ?? 0.05,
            mutation_chance: $data['mutation_chance'] ?? 0.01,
            dimensional_permeability: $data['dimensional_permeability'] ?? 1.0,
            entropy_cap: $data['entropy_cap'] ?? 1000.0,
            energy_conservation_factor: $data['energy_conservation_factor'] ?? 0.8
        );
    }
    ...
    /**
     * Interpolate towards another profile (Terraforming logic)
     */
    public function drift(PhysicsProfile $target, float $amount): self
    {
    ...
    /**
     * Calculate Euclidean distance between this profile and a baseline, normalized.
     * Higher score = more "drifted" from origin.
     */
    public function calculateDrift(self $baseline): float
    {
        $vars = [
            ...
            // 'entropy_cap', // Skip cap as it has huge range (1000-10000)
        ...
        return min(1.0, sqrt($totalDiff) / count($vars)); // Simple normalization
    }

    private function validate(): void
    {
        if ($this->instability_rate < 0) throw new InvalidArgumentException("Instability rate cannot be negative");
        if ($this->decay_rate < 0 || $this->decay_rate > 1) throw new InvalidArgumentException("Decay rate must be between 0 and 1");
        if ($this->mutation_chance < 0 || $this->mutation_chance > 1) throw new InvalidArgumentException("Mutation chance must be between 0 and 1");
        if ($this->energy_conservation_factor < 0 || $this->energy_conservation_factor > 1) throw new InvalidArgumentException("Energy conservation factor must be between 0 and 1");
    }
}
