...
class HeroAttractor
{
     ...
     */
    public function evaluateSpawn(CivilizationState $state, string $seed): ?array
    {
        $rng = hexdec(substr(md5($seed), 0, 8)) / 0xffffffff;
        
        // 1. Hero of Chaos (Entropy + Pressure)
        if ($state->internalEntropy > 0.7 && $state->militaryPressure > 0.6) {
            if ($rng < 0.3) {
     ...
     */
    private function createHero(CivilizationState $state, string $archetype, string $seed): array
    {
