 ...
 *
 * Contradiction (mâu thuẫn) tích lũy thành áp suất.
 * Supports both legacy linear contradiction index and non-linear formula:
 *   BT = α·PowerImbalance + β·ResourceStress + γ·IdeologyDivergence + δ·SocialFragmentation
 *   FA = k·BT²  (Feedback Amplification)
 *   CIT = (PowerImbalance×ResourceStress) + (IdeologyDivergence×SocialFragmentation)
 *   Raw = BT + FA + CIT;  P(t) = λ·P(t-1) + (1-λ)·Raw  (inertia)
 */
class PressureAccumulationField
{
    ...
    /** Accumulated pressure (monotonic increase when no release) — legacy */
    ...
    /**
     * Smoothed pressure with inertia: P(t) = λ·P(t-1) + (1-λ)·Raw.
     */
    ...
    /**
     * Analysis snapshot: vector trạng thái dùng cho phân tích ngoài (AI, dashboard).
     * Map dimensions WorldOS → coherence, entropy, belief_mass, resource_flow, stability, innovation_rate, contradiction_index.
     *
    ...
    public function getAnalysisSnapshot(WorldStateVector $s): array
    {
        ...
        return [
            ...
            'belief_mass' => min(1.0, $cohesion * 0.6 + $legitimacy * 0.4), // Tập hợp ý thức / niềm tin hệ thống
            'resource_flow' => $s->getResourceStock(), // Proxy: stock ≈ capacity to flow
