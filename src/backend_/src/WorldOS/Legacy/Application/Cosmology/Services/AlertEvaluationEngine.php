...
use WorldOS\Legacy\Domain\Cosmology\ValueObject\AlertRule;
 ...
 *   - Escalation: 3× WARNING → promote to CRITICAL
 *   - Composite: SSI_WARNING + COLLAPSE_IMMINENT → SYSTEMIC_INSTABILITY
 *   - Auto-action dispatch (returns action codes, does not execute)
 */
class AlertEvaluationEngine
{
    /** @var array<string, int> code → last fired epoch */
    private array $cooldowns = [];

    /** @var array<string, int> code → consecutive fire count */
    private array $streaks = [];

    private const ESCALATION_THRESHOLD = 3; // 3 consecutive → escalate
    ...
    public function evaluate(MetricsSnapshot $metrics, int $currentEpoch, array $extraMetrics = []): array
    {
        ...
        foreach ($rules as $rule) {
            ...
            if ($triggered) {
                ...
                // Escalation: 3 consecutive triggers → promote severity
    ...
    private function detectComposites(array $alerts, int $epoch): array
    {
        ...
        if (in_array('SSI_WARNING', $codes) && in_array('COLLAPSE_IMMINENT', $codes)) {
            $composites[] = [
                ...
                'epoch' => $epoch,
                'description' => 'Multiple instability indicators co-occurring — systemic crisis',
            ];
        }
        ...
        if (in_array('SSI_CRITICAL', $codes) && in_array('COLLAPSE_IMMINENT', $codes)) {
            $composites[] = [
                ...
                'epoch' => $epoch,
                'description' => 'Critical instability — immediate intervention required',
            ];
        }
