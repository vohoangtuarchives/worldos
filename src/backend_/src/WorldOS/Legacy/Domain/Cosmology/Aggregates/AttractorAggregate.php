...
namespace WorldOS\Legacy\Domain\Cosmology\Aggregates;
 ...
 * - Incarnation tree (parent → child chain)
...
class AttractorAggregate
{
    ...
    {
        $agg = new self();
        $agg->code = $attractor->code;
        $agg->name = $attractor->name;
        ...
        $agg->basinRadius = 0.3;
        $agg->curvature = 1.0;
        $agg->bifurcationThreshold = $attractor->bifurcationThreshold;
        $agg->recoveryRate = 0.02;
        $agg->rigidityThreshold = 0.7;

        // Create initial incarnation
        $inc = AttractorIncarnation::fromAttractor($attractor, $startTick);
        $agg->incarnations[] = $inc;
        $agg->incarnationCounter = 1;
    ...
    /**
     * Promote lifecycle: EMERGENT → DOMINANT.
     */
    ...
    /**
     * Decline lifecycle: DOMINANT → DECLINING.
     */
    ...
    /**
     * Memory biases parameters — NOT overrides physics.
     * From RFC §5.4:
     *   Basin Elasticity: r' = r × (1 + elasticity_factor)
     *   Recovery Speed:   ρ' = ρ × (1 + rg_factor)
     *   Rigidity:         θ' = θ × (1 - collapse_factor)
     */
     ...
     * High IKI → easier grandness but deeper chaos when it comes.
     */
