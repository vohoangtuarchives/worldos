...
use WorldOS\Evolution\Domain\Legacy\Contract\AttractorRepositoryInterface;
...
class CosmicEvolutionService
{
     ...
     */
    public function step(\WorldOS\Legacy\Domain\Cosmology\ValueObject\PhysicalLaws $laws, CosmicState $currentState, float $civilizationResonance = 0.0, int $deltaYears = 100, float $entropyFeedback = 0.0): CosmicState
    {
     ...
     */
    private function selectTargetAttractor(CosmicState $state, \WorldOS\Evolution\Domain\Legacy\Entity\AttractorAggregate $currentAttractor): string
    {
        ...
        $catalog = \WorldOS\Evolution\Domain\Legacy\ValueObject\Attractor::catalog();
        ...
        foreach ($catalog as $candidateCode => $candidate) {
            ...
            $equilibriumEnergy = $centroid[\WorldOS\Evolution\Domain\Legacy\ValueObject\WorldStateVector::DIMENSION_ORDER] ?? 0.5; // using Order as proxy for energy here in V3
