...
class EventEngine
{
    ...
    private function determineEventType(CivilizationPhase $phase, CivilizationSnapshot $state, string $seed, array $pressures = [], float $totalFactionPower = 1.0): string
    {
        ...
        if ($state->historyPhase === \WorldOS\Evolution\Domain\Legacy\Enum\CivilizationPhase::ILLUMINATION) {
        ...
        if ($phaseDetector->detect($state) === \WorldOS\Evolution\Domain\Legacy\Service\CivilizationPhaseDetector::PHASE_CHAOS) {
