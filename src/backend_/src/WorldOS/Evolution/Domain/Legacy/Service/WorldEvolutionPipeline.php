...
class WorldEvolutionPipeline
{
    ...
    public function __construct(
        ...
        private \WorldOS\Legacy\Domain\Material\Service\MaterialEvolutionService $materialService,
        private \WorldOS\Evolution\Domain\Legacy\Contract\EntropyLedgerInterface $entropyLedger,
        private PerturbationGenerator $perturbationGenerator, // NEW
        private ?\WorldOS\Evolution\Domain\Legacy\Contract\AttractorRepositoryInterface $attractorRepository = null,
     ...
     */
    public function step(\WorldOS\Legacy\Domain\Cosmology\Entity\World $world, WorldSnapshot $current, float $agentPerturbation = 0.0, int $deltaYears = 100, array $externalModifiers = []): WorldSnapshot
    {
        ...
        $tempWorld = new \WorldOS\Evolution\Domain\Legacy\Entity\WorldState($world->getId(), $nextCosmic, $nextEnvironment, $current->year + $deltaYears);
        ...
        $stateMachine = new \WorldOS\Evolution\Domain\Legacy\Service\HistoryStateMachine();
        ...
        if ($transcendenceEvent) {
            ...
            $phase = \WorldOS\Evolution\Domain\Legacy\Enum\CivilizationPhase::ILLUMINATION;
        }

        if ($lifecycleState === CivilizationLifecycleState::COLLAPSE || $macroState === \WorldOS\Evolution\Domain\Legacy\Service\HistoryStateMachine::STATE_COLLAPSE) {
             $this->lastStepEvents[] = [
                'id' => \Illuminate\Support\Str::uuid()->toString(),
        ...
        $nextPhase = $isTranscendence ? \WorldOS\Evolution\Domain\Legacy\Enum\CivilizationPhase::ILLUMINATION : $this->phaseEngine->determinePhase($nextCivilization);
        
        // If we are already illuminated, keep it until tension gets too high
        if ($currentPhase === \WorldOS\Evolution\Domain\Legacy\Enum\CivilizationPhase::ILLUMINATION && !$isTranscendence) {
            if ($nextCivilization->narrativeTension > 0.4 || $nextCivilization->internalEntropy > 0.5) {
                ...
                // Stay illuminated
                $nextPhase = \WorldOS\Evolution\Domain\Legacy\Enum\CivilizationPhase::ILLUMINATION;
            }
        }
        ...
        $chronicleService = new \WorldOS\Evolution\Domain\Legacy\Service\ChronicleService();
        ...
        foreach ($chronicleEvents as $ce) {
            $this->lastStepEvents[] = [
                ...
                'type' => $ce->type,
                'name' => $ce->title,
                'intensity' => $ce->severity === \WorldOS\Evolution\Domain\Legacy\ValueObject\ChronicleEvent::SEVERITY_CRITICAL ? 1.0 : ($ce->severity === \WorldOS\Evolution\Domain\Legacy\ValueObject\ChronicleEvent::SEVERITY_HIGH ? 0.8 : 0.5),
     ...
     */
    private function applyTrauma(\WorldOS\Evolution\Domain\Legacy\ValueObject\CivilizationResidual $residual, array $trauma): \WorldOS\Evolution\Domain\Legacy\ValueObject\CivilizationResidual
    {
    ...
    private function calculateSocialModifiers(CivilizationSnapshot $civ): array
    {
        ...
        foreach ($civ->socialClasses as $class) {
            switch ($class->type) {
                case \WorldOS\Evolution\Domain\Legacy\Enum\SocialClassType::MERCHANT:
                    // High Merchant power boosts efficiency
                    $mods['efficiency_bonus'] += $class->power * 0.2;
                    break;
                case \WorldOS\Evolution\Domain\Legacy\Enum\SocialClassType::WARRIOR:
                    // Disgruntled warriors hurt stability, happy ones help
                    $mods['stability_modifier'] += ($class->contentment - 0.5) * $class->power * 0.1;
                    break;
                case \WorldOS\Evolution\Domain\Legacy\Enum\SocialClassType::INTELLECTUAL:
                    // Intellectuals boost knowledge growth
                    $mods['knowledge_growth_factor'] += $class->power * 0.5;
                    break;
                case \WorldOS\Evolution\Domain\Legacy\Enum\SocialClassType::NOBILITY:
