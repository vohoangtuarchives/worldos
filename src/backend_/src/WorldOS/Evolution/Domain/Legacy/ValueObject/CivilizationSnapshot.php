...
final class CivilizationSnapshot
{
    ...
    public function __construct(
        ...
        /** @var \WorldOS\Evolution\Domain\Legacy\ValueObject\SocialClass[] */
        public readonly array $socialClasses = [],
        /** @var \WorldOS\Evolution\Domain\Legacy\ValueObject\Faction[] */
        public readonly array $factions = [],
        /** @var \WorldOS\Evolution\Domain\Legacy\ValueObject\PopulationCluster[] */
    ...
    public static function defaultObservation(int $year = 0): self
    {
        return new self(
            ...
            yearsInPhase: 0,
            socialClasses: [
                new \WorldOS\Evolution\Domain\Legacy\ValueObject\SocialClass(\WorldOS\Evolution\Domain\Legacy\Enum\SocialClassType::NOBILITY, 0.8, 0.9, 0.05),
                new \WorldOS\Evolution\Domain\Legacy\ValueObject\SocialClass(\WorldOS\Evolution\Domain\Legacy\Enum\SocialClassType::PRIESTHOOD, 0.6, 0.8, 0.05),
                new \WorldOS\Evolution\Domain\Legacy\ValueObject\SocialClass(\WorldOS\Evolution\Domain\Legacy\Enum\SocialClassType::WARRIOR, 0.5, 0.7, 0.10),
                new \WorldOS\Evolution\Domain\Legacy\ValueObject\SocialClass(\WorldOS\Evolution\Domain\Legacy\Enum\SocialClassType::MERCHANT, 0.2, 0.5, 0.05),
                new \WorldOS\Evolution\Domain\Legacy\ValueObject\SocialClass(\WorldOS\Evolution\Domain\Legacy\Enum\SocialClassType::PEASANTRY, 0.1, 0.4, 0.75),
                new \WorldOS\Evolution\Domain\Legacy\ValueObject\SocialClass(\WorldOS\Evolution\Domain\Legacy\Enum\SocialClassType::INTELLECTUAL, 0.01, 0.9, 0.01),
            ],
            factions: [
                new \WorldOS\Evolution\Domain\Legacy\ValueObject\Faction(
                    id: 'fac_initial',
                    name: 'Initial Ruling Council',
                    ideology: new \WorldOS\Evolution\Domain\Legacy\ValueObject\IdeologyVector(0.5, 0.5, 0.5, 0.5, 0.5, 0.5),
                    powerShare: 1.0,
                    cohesion: 0.8,
                    legitimacyClaim: 0.9
                )
            ...
            populationClusters: [
                new \WorldOS\Evolution\Domain\Legacy\ValueObject\PopulationCluster(
                    ideology: new \WorldOS\Evolution\Domain\Legacy\ValueObject\IdeologyVector(0.5, 0.5, 0.5, 0.5, 0.5, 0.5),
                    share: 1.0,
                    radicalization: 0.1,
            ...
            eliteNetwork: new \WorldOS\Evolution\Domain\Legacy\ValueObject\EliteNetwork(),
    ...
    public function evolve(
        \WorldOS\Legacy\Domain\Cosmology\ValueObject\PhysicalLaws $laws,
        ...
        $constraintEngine  = new \WorldOS\Evolution\Domain\Legacy\Service\RealityConstraintEngine();
        $dynamicsAnalyzer  = new \WorldOS\Evolution\Domain\Legacy\Service\DynamicsAnalyzer();
        $attractorService  = new \WorldOS\Evolution\Domain\Legacy\Service\AttractorService();
        $phaseEngine       = new \WorldOS\Evolution\Domain\Legacy\Service\PhaseEngine();
        $kernel            = new \WorldOS\Evolution\Domain\Legacy\Mathematics\DynamicalKernel();
        $narrativeEngine   = new \WorldOS\Evolution\Domain\Legacy\Service\NarrativeEngine();
        $heroSystem        = new \WorldOS\Evolution\Domain\Legacy\Service\HeroSystem();
        $attractorModifier = new \WorldOS\Evolution\Domain\Legacy\Service\AttractorFieldModifier();
        $phaseDetector     = new \WorldOS\Evolution\Domain\Legacy\Service\CivilizationPhaseDetector();

        $populationDynamics = new \WorldOS\Evolution\Domain\Legacy\Service\PopulationDynamicsEngine();
        $polarizationEngine = new \WorldOS\Evolution\Domain\Legacy\Service\PolarizationEngine();
        $legitimacyEngine   = new \WorldOS\Evolution\Domain\Legacy\Service\LegitimacyEngine();
        $factionEvolution   = new \WorldOS\Evolution\Domain\Legacy\Service\FactionEvolutionEngine();
        $ecoPressureEngine  = new \WorldOS\Evolution\Domain\Legacy\Service\EcologicalPressureEngine();
        $forecastEngine     = new \WorldOS\Evolution\Domain\Legacy\Service\ForecastEngine();
        $policyAdvisor      = new \WorldOS\Evolution\Domain\Legacy\Service\PolicyAdvisor();
        $eliteDecision      = new \WorldOS\Evolution\Domain\Legacy\Service\EliteDecision();
        
        $phaseForces = $phaseEngine->getPhaseForces($this->historyPhase);
        
        $state = \WorldOS\Evolution\Domain\Legacy\ValueObject\StateVector::fromSnapshot($this);
        $prevSnapshot = $this;
        ...
        $currentNetwork = $this->eliteNetwork ?? new \WorldOS\Evolution\Domain\Legacy\ValueObject\EliteNetwork();
        $currentStructuralEntropy = $this->structuralEntropy;
        
        for ($i = 0; $i < $deltaYears; $i++) {
            $ieNow = $state->values[array_search('ie', \WorldOS\Evolution\Domain\Legacy\ValueObject\StateVector::KEYS)] ?? $this->internalEntropy;
            
            // Build temporary snapshot to evaluate current phase
            $tempCivForPhase = \WorldOS\Evolution\Domain\Legacy\ValueObject\CivilizationSnapshot::fromArray(
            ...
            if ($heroSystem->checkEmergence($totalTension, 1, $ieNow, $currentBasinPhase)) {
                ...
                foreach (\WorldOS\Evolution\Domain\Legacy\ValueObject\StateVector::KEYS as $idx => $keyName) {
                    if (isset($heroImpulses[$idx]) && $heroImpulses[$idx] != 0) {
                        $sValues[$keyName] = max(-1.0, min(1.0, $sValues[$keyName] + $heroImpulses[$idx]));
                    }
                }
                $state = new \WorldOS\Evolution\Domain\Legacy\ValueObject\StateVector($sValues);
            ...
            // AI Forecast & Policy
            $ceVal = $state->values[array_search('ce', \WorldOS\Evolution\Domain\Legacy\ValueObject\StateVector::KEYS)] ?? $this->culturalEnergy;
            $techVal = $state->values[array_search('tech', \WorldOS\Evolution\Domain\Legacy\ValueObject\StateVector::KEYS)] ?? $this->technologicalLevel;
            $infoVal = $state->values[array_search('info', \WorldOS\Evolution\Domain\Legacy\ValueObject\StateVector::KEYS)] ?? $this->informationFlow;
            ...
            if ($chosenPolicy === \WorldOS\Evolution\Domain\Legacy\Service\PolicyAdvisor::POLICY_TRIGGER_REFORM) {
                $ext['ie'] = ($ext['ie'] ?? 0) - 0.1; // Reduce internal entropy
                $ext['ce'] = ($ext['ce'] ?? 0) + 0.05; // Boost cultural energy
            } elseif ($chosenPolicy === \WorldOS\Evolution\Domain\Legacy\Service\PolicyAdvisor::POLICY_REDUCE_CENTRALIZATION) {
                $ext['stability'] = ($ext['stability'] ?? 0) + 0.02; // Minor stability buff
            }

            // ── PHASE 10: POPULATION DYNAMICS & FACTION EVOLUTION ────────────
            $ieIdx = array_search('ie', \WorldOS\Evolution\Domain\Legacy\ValueObject\StateVector::KEYS);
            ...
            $scVal = $state->values[array_search('sc', \WorldOS\Evolution\Domain\Legacy\ValueObject\StateVector::KEYS)] ?? $this->spiritualCohesion;
            $ceVal = $state->values[array_search('ce', \WorldOS\Evolution\Domain\Legacy\ValueObject\StateVector::KEYS)] ?? $this->culturalEnergy;
            ...
            $v = $nextState->values;
            $legIdx = array_search('legitimacy', \WorldOS\Evolution\Domain\Legacy\ValueObject\StateVector::KEYS);
            $ecIdx  = array_search('eliteCohesion', \WorldOS\Evolution\Domain\Legacy\ValueObject\StateVector::KEYS);
            $ineqIdx = array_search('inequality', \WorldOS\Evolution\Domain\Legacy\ValueObject\StateVector::KEYS);
            $ceIdx  = array_search('ce', \WorldOS\Evolution\Domain\Legacy\ValueObject\StateVector::KEYS);
            ...
            $nextState = new \WorldOS\Evolution\Domain\Legacy\ValueObject\StateVector($nextStateValues);
            ...
            $stab = $calcStab;
            $p = $nextState->values[array_search('prosperity', \WorldOS\Evolution\Domain\Legacy\ValueObject\StateVector::KEYS)];
            $ieNew = $nextState->values[array_search('ie', \WorldOS\Evolution\Domain\Legacy\ValueObject\StateVector::KEYS)];
            ...
            $legacyIdx = array_search('legacy', \WorldOS\Evolution\Domain\Legacy\ValueObject\StateVector::KEYS);
            ...
            if ($fragility > 1.5) {
                ...
                $nextState = new \WorldOS\Evolution\Domain\Legacy\ValueObject\StateVector($shockStateValues);
    ...
    public static function fromArray(array $data): self
    {
        return new self(
            ...
            socialClasses: array_map(
                fn($c) => \WorldOS\Evolution\Domain\Legacy\ValueObject\SocialClass::fromArray($c), 
                $data['social_classes'] ?? []
            ),
            factions: array_map(
                fn($f) => \WorldOS\Evolution\Domain\Legacy\ValueObject\Faction::fromArray($f),
                $data['factions'] ?? []
            ),
            populationClusters: array_map(
                fn($p) => \WorldOS\Evolution\Domain\Legacy\ValueObject\PopulationCluster::fromArray($p),
                $data['population_clusters'] ?? []
            ),
            eliteNetwork: isset($data['elite_network']) ? \WorldOS\Evolution\Domain\Legacy\ValueObject\EliteNetwork::fromArray($data['elite_network']) : null,
