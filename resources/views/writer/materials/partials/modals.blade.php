...
<div id="strengthModal{{ $instance->id }}" class="hidden fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-middle bg-gray-800 rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full border border-white/10">
            <form method="POST" action="{{ route('materials.adjust-strength', $instance->id) }}">
                @csrf
...
{{-- Mutation Modal --}}
<div id="mutateModal{{ $instance->id }}" class="hidden fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <div class="fixed inset-0 bg-gray-900/90 backdrop-blur-sm transition-opacity" aria-hidden="true" onclick="document.getElementById('mutateModal{{ $instance->id }}').classList.add('hidden')"></div>
        <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
        <div class="inline-block align-middle bg-gray-800 rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full border border-white/10">
            <form method="POST" action="{{ route('materials.force-mutation', $instance->id) }}">
                @csrf
                <div class="px-6 py-6 sm:px-8">
                    <div class="flex items-center justify-between mb-6">
                        <h3 class="text-xl font-bold text-white tracking-tight">Cưỡng bức Đột biến</h3>
                        <button type="button" onclick="document.getElementById('mutateModal{{ $instance->id }}').classList.add('hidden')" class="text-gray-500 hover:text-white transition-colors">
                            <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
                        </button>
                    </div>
                ...
                <div class="bg-gray-900/50 px-6 py-4 sm:px-8 flex flex-row-reverse space-x-reverse space-x-3">
                    ...
                    <button type="button" onclick="document.getElementById('mutateModal{{ $instance->id }}').classList.add('hidden')" 
                            class="inline-flex items-center justify-center rounded-xl bg-white/5 px-6 py-2.5 text-sm font-bold text-gray-400 hover:bg-white/10">
                        Hủy
                    </button>
                </div>
