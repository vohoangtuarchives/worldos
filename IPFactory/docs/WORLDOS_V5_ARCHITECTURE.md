...
*Bản gốc design: `docs/plans/2025-02-26-worldos-v5-design.md`*
...
với $\mathbf{T}_{17} \in [0,1]^{17}$ là vector 17 chiều trait, $\mathbf{w} \in \mathbb{R}^{17}$ là Context Weight Vector (cùng chiều với $\mathbf{T}_{17}$ để tích vô hướng cho scalar).

Agent khởi tạo deterministically từ Macro State + Universe Seed.

### 3.5. Pressure & Residuals

CivilizationResidual lưu "sẹo" (war_trauma, …); decay theo thời gian; còn tồn tại thì cộng vào Áp lực Xã hội. **Pressure** = $f(\text{inequality}, \text{entropy}, \text{trauma}, \text{MaterialStress})$; công thức cụ thể (tổ hợp tuyến tính hoặc phi tuyến) trong spec triển khai.
...
Mỗi Zone: `base_mass`, `structured_mass`, `free_energy`, `entropy`. **Invariant:** $\text{structured\_mass} \le \text{base\_mass}$. *Lưu ý:* Trong doc này, **entropy** là biến trạng thái chuẩn hóa [0,1] (độ rối loạn / độ dễ vỡ của vùng), không phải entropy nhiệt động lực học (J/K); toàn cục tuân Axiom M2 (entropy không biến mất, chỉ tích tụ/chuyển vùng).

**Biến đổi vật lý:**
- **Tổ chức (Organization):** base → structured; cần `extraction_rate`, `stability`, `tech_efficiency`. Hiệu ứng: $\text{entropy} \mathrel{+}= k_1 \cdot \Delta\text{structured}$ (tăng “độ dễ vỡ” khi thêm cấu trúc).
...
*Chú thích:* `base_mass depletion ratio` = tỉ lệ phần base_mass đã bị “cạn kiệt” hoặc không còn khả dụng (vd. $(1 - \text{structured\_mass}/\text{base\_mass})$ hoặc metric tương đương; base_mass bản thân bất biến). `free_energy` dự trữ cho mô hình năng lượng, chưa xuất hiện trong công thức ở bản spec này.
...
Khi **Structural Coherence Index (SCI)** $< \text{CriticalThreshold}$ (có stochastic nhỏ) → trigger Meta-Cycle. SCI = chỉ số mạch lạc cấu trúc (vd. hàm nghịch biến của Global Entropy, thuận biến của độ kết nối); định nghĩa đầy đủ trong spec triển khai. Hiệu ứng: $\approx 80\%$ StructuredMaterial sụp đổ; $\approx 50\%$ EmbodiedKnowledge quét; BaseMaterial không mất; KnowledgeCoreSignature bẻ cong seed kỷ nguyên sau (lịch sử lặp lại "có âm vang").

### 4.4. Cultural State Vector ($C_z$)

Vector văn hóa Zone $z$: $C_z(t) \in [0,1]^k$ ($k$ = 5–8 chiều); thành phần được clamp hoặc drift đủ chậm để giữ trong [0,1]. Các chiều ví dụ: Tradition Rigidity, Innovation Openness, Collective Trust, Violence Tolerance, Institutional Respect, Myth Intensity.

**Động lực:**
- **Drift nội sinh chậm:** $C_z(t+1) = C_z(t) + \epsilon \cdot \text{InternalDynamics}$ ($\epsilon$ nhỏ).
- **Ảnh hưởng sự kiện:** $\Delta C_{\text{event}}$ kẹp trong [0,1]; inertia khi $C_z \to 0$ hoặc $1$.
- **Lan truyền không gian:** $C_z(t+1) \mathrel{+}= \beta \sum_{\text{neighbor}} (C_{\text{neighbor}} - C_z)$; $\beta$ đủ nhỏ hoặc clamp để $C_z \in [0,1]^k$.
...
CivilizationResidual: sẹo (vd. war_trauma) + decay; còn thì cộng Pressure. Myth Scar: Entity lớn chết → trường (ideology_vector_snapshot, emotional_intensity, trauma_level, symbolic_power). **Lan tỏa:** propagation kernel $\propto \exp(-\gamma \cdot \text{distance})$ ($\gamma > 0$; distance = khoảng cách trên đồ thị/topology Zone); vùng scar cao khó ổn định.
...
với $a,b,c \ge 0$; $D_z$ = Cultural Divergence so với thủ đô, $S_z$ = Stress chính trị/kinh tế vật chất (các đại lượng chuẩn hóa hoặc cùng thang đo).

Giai đoạn: Stable → Agitating ($P_z > T_{\text{agitate}}$) → Destabilized → Split ($P_z > T_{\text{split}}$ đủ thời gian $\tau_2$). Split chỉ đổi Owner_Regime và biên giới; **không tạo Zone mới** (topology cố định O(E)). *Fixed Topology* áp dụng ở tầng logic mô phỏng; `ZoneCommand::Spawn`/`Destroy` trong Rust dùng cho khởi tạo hoặc trường hợp đặc biệt, không dùng trong vòng tick bình thường.
...
- $C_z(t) \in [0,1]^k$, $k$ = 5–8 (thành phần clamp hoặc drift đủ chậm).
- **Drift:** $C_z(t+1) = C_z(t) + \epsilon \cdot \text{InternalDynamics}$, $\epsilon$ nhỏ.
- **Lan truyền:** $C_z(t+1) \mathrel{+}= \beta \sum_{\text{neighbor}} (C_{\text{neighbor}} - C_z)$; $\beta$ nhỏ hoặc clamp.
- $\Delta C$ từ sự kiện kẹp trong [0,1]; inertia khi $C_z \to 0$ hoặc $1$.

### C.5. Secession Pressure

$$ P_z = a \cdot D_z + b \cdot S_z - c \cdot \text{InstitutionalTrust}_z, \qquad a,b,c \ge 0 $$
...
Propagation kernel theo khoảng cách (đồ thị/topology): $\gamma > 0$
...
## Phụ lục D. Kiểm tra kiến thức — Lỗ hổng đã xử lý & Lưu ý

Kiểm tra tính nhất quán toán học và vật lý đã được thực hiện; các điểm dưới đây đã được bổ sung hoặc làm rõ trong bản hiện tại.

| Vấn đề | Xử lý |
|--------|--------|
| **Entropy** dễ nhầm với entropy nhiệt động (J/K) | Thêm chú thích: trong doc, entropy là biến trạng thái [0,1] (độ rối loạn/dễ vỡ); toàn cục vẫn tuân Axiom M2. |
| **base_mass depletion ratio** — base_mass bất biến | Giải thích: tỉ lệ phần "cạn kiệt" hoặc không khả dụng (vd. $(1 - \text{structured}/\text{base})$); base_mass không đổi. |
| **$C_z \in \mathbb{R}^k$** nhưng delta/inertia [0,1] | Chuẩn hóa: $C_z \in [0,1]^k$, thêm clamp / $\epsilon$, $\beta$ nhỏ. |
| **Context Weight $\mathbf{w}$** — chiều không nêu | Ghi rõ $\mathbf{w} \in \mathbb{R}^{17}$ để tích $\mathbf{T}_{17}^\top \mathbf{w}$ là scalar. |
| **free_energy** không xuất hiện trong công thức | Ghi là dự trữ cho mô hình năng lượng; spec triển khai sẽ định nghĩa. |
| **ZoneCommand Spawn/Destroy** vs Fixed Topology | Làm rõ: Fixed Topology ở tầng logic; ZoneCommand cho khởi tạo/trường hợp đặc biệt, không dùng trong tick bình thường. |
| **Pressure** không có công thức | Thêm $f(\text{inequality}, \text{entropy}, \text{trauma}, \text{MaterialStress})$; công thức cụ thể trong spec triển khai. |
| **SCI** không định nghĩa | Thêm: SCI = chỉ số mạch lạc cấu trúc (vd. nghịch biến Global Entropy, thuận biến connectivity); chi tiết trong spec triển khai. |
| **Secession $P_z$** — dấu hệ số | Ghi $a,b,c \ge 0$ để high trust giảm $P_z$, high $D_z$/$S_z$ tăng $P_z$. |
| **Myth Scar propagation** — "distance" | Làm rõ: distance = khoảng cách trên đồ thị/topology Zone; $\gamma > 0$. |

**Kết luận:** Không phát hiện mâu thuẫn logic hoặc toán học nội tại. Các công thức nhất quán với tài liệu gốc (Material/Knowledge, Historical Accumulation). Triển khai cần định nghĩa rõ: Pressure, SCI, CriticalThreshold, và các hằng số $k_1$, $\epsilon$, $\beta$, $\gamma$, $a,b,c$.
